...
class ChatPresenter{
  ...
  void getChatHistory(String accessCode){
    _repository.getChatHistory(accessCode).then((c) => _view.showChatHistory(c.data))
        .catchError((e) => print(e.toString()));
  }

  void getMoreChatHistory(String accessCode,int page){
    _repository.getMoreChatHistory(accessCode,page).then((c) => _view.showMoreChatHistroy(c.data))
        .catchError((e) => print(e.toString()));
  }

  void sendMessage(String accessCode,String mesg,int articleId,int doctorId,int serviceId){
    _repository.sendMessage(accessCode, mesg, articleId, doctorId, serviceId).then((m) => _view.showNewMessage(m))
        .catchError((e) => print('Send Message Error ${e.toString()}'));
  }

  void sendImage(String accessCode,String filePath,int articleId,int doctorId,int serviceId){
    _repository.sendImage(accessCode, filePath, articleId, doctorId, serviceId).then((m) => _view.showNewMessage(m))
        .catchError((e) => print(e.toString()));
  }
